...
export class Skills extends React.Component<SkillProps, SkillState> {
    ...
    return (
      <div className={styles.container}>
        <div>
          ...
          <div className={styles.divResponse}>You typed: {this.state.addSkill}</div>
        </div>
        <ul>
          {this.state.skills.map((item: Skill) => {
            return (
              <li key={item.Id} onClick={() => this.removeSkill(item)} className="li-skills">
                {item.Title}
              </li>
            );
          })}
  ...
  private addSkillToSP(skill: Skill) {
    ...
    return this.props.context.spHttpClient.post(qry, SPHttpClient.configurations.v1, {
