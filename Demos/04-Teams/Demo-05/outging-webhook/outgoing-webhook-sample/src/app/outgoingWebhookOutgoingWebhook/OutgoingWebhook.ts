...
export class OutgoingWebhook implements IOutgoingWebhook {
    ...
    private static processAuthenticatedRequest(incomingText: string): Partial<builder.Activity> {
        ...
        // load planets -> mocking a service call
        ...
        if (!selectedPlanet || !selectedPlanet.length) {
            ...
            const adaptiveCard = OutgoingWebhook.getPlanetDetailCard(selectedPlanet[0]);
            message.type = 'result';
    ...
    public requestHandler(req: express.Request, res: express.Response, next: express.NextFunction) {
        ...
        if (securityToken && securityToken.length > 0) {
            ...
            if (msgHash === auth) {
                ...
                const scrubbedText = OutgoingWebhook.scrubMessage(incoming.text);
                message = OutgoingWebhook.processAuthenticatedRequest(scrubbedText);
            } else {
