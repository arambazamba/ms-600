...
/**
 * Implementation for OutgoingWebhookApp Outgoing Webhook
 */
@OutgoingWebhookDeclaration("/api/webhook")
export class OutgoingWebhookAppOutgoingWebhook implements IOutgoingWebhook {
  /**
   * The constructor
   */
  public constructor() {}
  ...
  public requestHandler(req: express.Request, res: express.Response, next: express.NextFunction) {
    ...
    if (securityToken && securityToken.length > 0) {
      ...
      if (msgHash === auth) {
        // scrub the message and process the request to return the result
        const scrubbedText = OutgoingWebhookAppOutgoingWebhook.scrubMessage(incoming.text);
        message = OutgoingWebhookAppOutgoingWebhook.processAuthenticatedRequest(scrubbedText);
      } else {
  ...
  private static processAuthenticatedRequest(incomingText: string): Partial<builder.Activity> {
    ...
    if (!selectedPlanet || !selectedPlanet.length) {
      ...
      const adaptiveCard = OutgoingWebhookAppOutgoingWebhook.getPlanetDetailCard(selectedPlanet[0]);
      message.type = "result";
